...
K = 0;
E = 0;
dz = 0.7788*wire_radius; %for circular wire, check maxwell paper for other geometries
%{
for zprim = (0:length/(turns-1):length)

    for z = (0:length/(turns-1):(length))
        if z==zprim %singular point handled as instructed in Elektricitetsl�ra by Hall�n 1953
...
for zprim = (0:length/(turns-1):length)
    ...
    for z = (dz:length/(turns-1):(length+dz))
